...
public class ArcaneSpraySpell : BaseSpell
{
    ...
    public override IEnumerator Cast(Transform where, Vector3 target, Hittable.Team team, int damage, float speed, string trajectory) {
        ...
        for (int i = 0; i < N; i++) {
            GameManager.Instance.projectileManager.CreateProjectile(p.sprite, trajectory, where.position, RotateVector((Random.value*2-1)*angle, target - where.position), RPN.Eval(p.speed, null), OnHit(damage), RPN.Eval(p.lifetime, null));
        }
