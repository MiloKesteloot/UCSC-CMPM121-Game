...
public class ArcanePulseSpell : BaseSpell
{
    ...
    public override IEnumerator Cast(Transform where, Vector3 target, Hittable.Team team, int damage, float speed, string trajectory) {
        ...
        for (int i = 0; i < N; i++) {
            GameManager.Instance.projectileManager.CreateProjectile(p.sprite, p.trajectory, where.position, RotateVector(360f/N*i), RPN.Eval(p.speed, null), OnHit((int) RPN.Eval(baseSpellInfo.damage.amount, null)), RPN.Eval(p.lifetime, null));
        }
